...
def settings_text(signature: str | None) -> str:
    sig_text = f"\n\n✍️ <b>امضای فعلی:</b>\n<code>{_e(signature)}</code>" if signature else "\n\n✍️ <i>امضایی تنظیم نشده است.</i>"
    return f"⚙️ <b>تنظیمات</b>{sig_text}"


def ask_signature() -> str:
    return (
        "✍️ <b>تنظیم زیرنویس (امضا)</b>\n\n"
        "متنی که می‌خواهید زیر تمام پیام‌های منتقل‌شده قرار بگیرد را بفرستید.\n\n"
        "⚠️ <i>دقت کنید که حداکثر طول کپشن عکس و ویدیو در تلگرام 1024 کاراکتر است.</i>\n\n"
        "برای بازگشت بدون تغییر، روی لغو کلیک کنید."
    )